...
int fbtft_write_spi(struct fbtft_par *par, void *buf, size_t len)
{
        struct spi_transfer t = {
                .tx_buf = buf,
                .len = len,
                .bits_per_word = par->pdata->display.buswidth,
        };
        struct spi_message m;
        int k;
	if (par->pdata->display.buswidth == 9){
		unsigned short* pshort;
		pshort = (unsigned short*)buf;
		for (k = 0; k < len / 2; k++){
			unsigned short tmp = *pshort;
			unsigned char n = (unsigned char)(tmp >> 0);
			n = (n << 4)|(n >> 4);  
			n = ((n << 2) & 0xcc) | ((n >> 2) & 0x33);  
			n = ((n << 1) & 0xaa) | ((n >> 1) & 0x55);
			tmp &= 0xff00;
